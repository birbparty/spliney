...
[[ -f "$asset" && -f "$oracle" && -d "$reference_dir" && -n "$output_dir" ]] || {
  echo "usage: run_exact_raylib_capture.sh --asset FILE --oracle FILE --reference-dir DIR --output-dir DIR" >&2
  exit 2
}
...
nim c --hints:off --mm:orc -d:useNaylib --out:"$binary" \
  "$project_dir/tools/verify_exact_raylib_capture.nim"
nim c --hints:off --mm:orc --out:"$vertex_binary" \
